...
def draw_points(image, points, color_palette='tab20', palette_samples=16, confidence_threshold=0.5):
    ...
    for i, pt in enumerate(points):
        ...
        # cv2.imwrite(f'save_image{i}.jpg', image)
        # 여기서 관절을 점을 찍어준다.
        # pt[0] is y coordinate; pt[1]is x coordinate.
        # so How many have pt parameters? # TODO
    return image, points
...
def draw_skeleton(image, points, skeleton, color_palette='Set2', palette_samples=8, person_index=0,
    ...
    for i, joint in enumerate(skeleton):
        ...
        # cv2.imwrite(f'save_image{i}.jpg', image)
        # 여기서 retunr되는 이미즈는 관절과 관절 사이 그려저 나오는 이미지임.
...
def draw_points_and_skeleton(image, points, skeleton, points_color_palette='tab20', points_palette_samples=16,
    ...
    image, points = draw_points(image, points, color_palette=points_color_palette, palette_samples=points_palette_samples,
                        confidence_threshold=confidence_threshold)
    return image, points
